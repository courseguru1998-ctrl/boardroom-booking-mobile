...
import { formatBookingTime, formatFullDate } from '../../utils/date';
...
const ROOM_COLORS = [
  ...
  { bg: '#F43F5E', light: '#FFE4E6', text: '#BE123C' },
...
export function CalendarScreen({ navigation }: MainTabScreenProps<'Calendar'>) {
  ...
  // Fetch bookings for the visible month (for calendar dots)
  const { data: monthBookings, isLoading: monthLoading, refetch: refetchMonth } = useBookings({
  ...
  const bookingsByDate = useMemo(() => {
    ...
    monthBookings?.data?.forEach((booking) => {
  ...
  const filteredByRoom = selectedRoomId
    ? monthBookings?.data?.filter((b) => b.roomId === selectedRoomId)
    : monthBookings?.data;

  const bookingsForSelectedDate = filteredByRoom?.filter((booking) => {
    const bookingDate = format(parseISO(booking.startTime), 'yyyy-MM-dd');
    return bookingDate === selectedDateStr;
  }) || [];

  // Today's bookings count
  const todayBookingsCount = monthBookings?.data?.filter((b) =>
  ...
  const renderBookingCard = (booking: Booking, index: number) => {
    ...
    return (
      ...
      >
        ...
        <View style={[styles.timeRow, { backgroundColor: colors.surfaceSecondary, borderRadius: 8 }]}>
        ...
        {booking.description && (
          <Text style={[styles.bookingDescription, { color: colors.textTertiary }]} numberOfLines={2}>
            {booking.description}
          </Text>
        )}
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={monthLoading} onRefresh={refetchMonth} />
        }
      >
        {/* Header */}
        ...
        <View style={styles.filterSection}>
          ...
          <TouchableOpacity
            ...
            style={[styles.filterButton, { backgroundColor: colors.surfaceSecondary, borderColor: colors.border }]}
          >
          ...
          {showRoomFilter && (
            <View style={[styles.filterOptions, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <TouchableOpacity
                ...
                style={[
                  styles.filterOption,
                  !selectedRoomId && { backgroundColor: colors.primaryLight },
                ]}
              >
              ...
              {rooms.map((room, index) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.filterOption,
                      isSelected && { backgroundColor: roomColor.light },
                    ]}
                  >
                    <View style={styles.filterOptionContent}>
                      <View style={[styles.roomColorDot, { backgroundColor: roomColor.bg }]} />
                      <View style={styles.filterOptionDetails}>
                        <Text style={[styles.filterOptionText, { color: colors.text }]}>
                          {room.name}
                        </Text>
            ...
            <Card style={styles.emptyCard}>
              <View style={styles.emptyContent}>
                ...
                <Text style={[styles.emptySubtitle, { color: colors.textTertiary }]}>
                  {selectedRoom ? `No bookings for ${selectedRoom.name} on this day` : 'No bookings for this day'}
                </Text>
              </View>
            </Card>
          ) : (
